...
`assets/js/scroll-animations.js` uses locally bundled GSAP 3.13.0 and ScrollTrigger from `assets/vendor/`. It adds staggered block reveals, heading wipes, and a reading-progress line. Phones use shorter movement and duration. Dashboard view changes reinitialize motion, and blog filters and FAQ changes refresh scroll positions. Reduced-motion preferences disable effects, including when changed during a visit. Content stays visible without JavaScript; a native animation fallback handles unavailable GSAP files.

Motion references: [GSAP Scroll](https://gsap.com/scroll/) and [CSS text animation examples](https://alvarotrigo.com/blog/css-text-animations/). Vendor license notices are retained in the bundled files.
