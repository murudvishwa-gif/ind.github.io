(() => {
  ...
  let profile = {};
  try { profile = JSON.parse(sessionStorage.getItem('stackly-profile')) || {}; } catch {}
  state.name = profile.name || 'Guest';
  state.email = profile.email || 'Sign in to display your email';
  ...
  identity.innerHTML = `
    <section class="profile-section" aria-labelledby="profile-personal-title">
      <h2 id="profile-personal-title">Personal details</h2>
      <strong>${esc(state.name)}</strong><span>${esc(state.email)}</span>
    </section>
    <section class="profile-section" aria-labelledby="profile-workspace-title">
      <h2 id="profile-workspace-title">Your workspace</h2>
      <dl><div><dt>Role</dt><dd>${admin ? 'Administrator' : 'Client'}</dd></div><div><dt>Workspace</dt><dd>${admin ? 'Operations management' : 'Client portal'}</dd></div><div><dt>Access</dt><dd>Demo session</dd></div></dl>
    </section>
    <section class="profile-section" aria-labelledby="profile-preferences-title">
      <h2 id="profile-preferences-title">Account preferences</h2>
      <a href="404.html">Notification preferences <span aria-hidden="true">↗</span></a>
      <a href="404.html">Privacy &amp; security <span aria-hidden="true">↗</span></a>
    </section>
    <section class="profile-section" aria-labelledby="profile-actions-title">
      <h2 id="profile-actions-title">Account actions</h2>
      <a href="404.html">Edit profile <span aria-hidden="true">↗</span></a>
      <a href="404.html">Manage account <span aria-hidden="true">↗</span></a>
    </section>`;
  identity.id = 'profile-details';
  ...
  profileButton.setAttribute('aria-controls', identity.id);
  profileButton.innerHTML = '<span class="avatar" aria-hidden="true">' + esc(state.name.split(/\s+/).map(n => n[0]).slice(0,2).join('').toUpperCase()) + '</span><span>Profile</span><span aria-hidden="true">⌄</span>';
  profileControl.append(profileButton, identity);
  document.querySelector('.top-actions > .avatar').replaceWith(profileControl);
  const closeProfile = () => { identity.hidden = true; profileButton.setAttribute('aria-expanded', 'false'); };
  profileButton.addEventListener('click', () => { identity.hidden = !identity.hidden; profileButton.setAttribute('aria-expanded', String(!identity.hidden)); });
  document.addEventListener('click', event => { if (!profileControl.contains(event.target)) closeProfile(); });
  document.addEventListener('focusin', event => { if (!profileControl.contains(event.target)) closeProfile(); });
  document.addEventListener('keydown', event => { if (event.key === 'Escape' && !identity.hidden) { closeProfile(); profileButton.focus(); } });
  document.addEventListener('click', event => {
    const button = event.target.closest('button');
    if (button && !button.matches('.mobile-menu, .profile-toggle, .workspace-menu-close')) {
      event.preventDefault(); event.stopImmediatePropagation(); window.location.assign('404.html');
    }
  }, true);
  document.addEventListener('submit', event => {
    event.preventDefault(); event.stopImmediatePropagation();
    if (event.target.reportValidity()) window.location.assign('404.html');
  }, true);
  let view = 'overview';
  let visibleRows = [];
  ...
  function overview(period = 'month') {
    ...
    document.getElementById('period').addEventListener('change', e => render('overview', e.target.value));
  }
  function records() {
  ...
  function render(next, period = 'month') {
    ...
    if(view==='overview')overview(period);
    ...
    const supplementary = {
      overview: [['Production priorities', 'Confirm drawing revisions before the next machining run.', 'Review incoming material documentation.', 'Coordinate inspection and delivery milestones.'], ['Upcoming milestones', 'Engineering review · 22 September', 'In-process inspection · 24 September', 'Dispatch preparation · 28 September']],
      projects: [['Production stages', 'Design review → material preparation → machining.', 'Inspection follows each critical operation.', 'Packaging begins after final acceptance.'], ['Project documentation', 'Approved drawings and revision history.', 'Material certificates and inspection plans.', 'Delivery requirements and packing instructions.']],
      requests: [['Before requesting a quote', 'Prepare drawings with dimensions and tolerances.', 'Specify material, quantity and surface finish.', 'Include your preferred delivery date.'], ['Quotation review', 'Technical feasibility and open design questions.', 'Material availability and process selection.', 'Production schedule and commercial review.']],
      billing: [['Payment overview', 'Pending invoices are listed above for review.', 'Match invoice references to your project records.', 'Sample amounts are shown in USD.'], ['Billing documents', 'Order references and agreed milestones.', 'Invoice details and supporting documentation.', 'Payment history and account reconciliation.']],
      team: [['Department coverage', 'Engineering · design and technical review.', 'Operations · planning and production.', 'Quality · inspection and documentation.'], ['Team coordination', 'Weekly production planning and workload review.', 'Drawing revisions shared before shift handover.', 'Inspection results reviewed before dispatch.']],
      settings: [['Account details', 'Name: ' + state.name, 'Email: ' + state.email, 'Workspace: ' + (admin ? 'Administrator' : 'Client')], ['Workspace preferences', 'Project updates and delivery notifications.', 'Document access and communication preferences.', 'Account management is unavailable in this demo.']]
    };
    content.insertAdjacentHTML('beforeend', '<div class="dashboard-extras">' + supplementary[view].map(([title,...items]) => '<section class="panel"><h2>' + esc(title) + '</h2><ul>' + items.map(item => '<li>' + esc(item) + '</li>').join('') + '</ul><a class="dashboard-detail" href="404.html">View details ↗</a></section>').join('') + '</div>');
    content.querySelectorAll('[data-view]').forEach(button => {
      const link = document.createElement('a'); link.href = '?view=' + button.dataset.view;
      link.dataset.view = button.dataset.view; link.className = button.className;
      link.textContent = button.textContent; button.replaceWith(link);
    });
    document.dispatchEvent(new Event('dashboard:view'));
  }
  document.addEventListener('click',event=>{
    const nav=event.target.closest('[data-view]');if(nav){event.preventDefault();render(nav.dataset.view);return;}
  ...
  render(new URLSearchParams(location.search).get('view') || 'overview');
})();
